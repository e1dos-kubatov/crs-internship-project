...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<PartnerOrderResponseDto>> createOrder(@Valid @RequestBody PartnerOrderRequestDto request,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("Partner order created successfully", carService.createOrder(request, principal.getName())));
    }

    @GetMapping("/my")
    @PreAuthorize("hasAnyRole('PARTNER', 'ADMIN')")
    public ResponseEntity<ApiResponse<List<PartnerOrderResponseDto>>> getMyOrders(Principal principal) {
        return ResponseEntity.ok(ApiResponse.success("Partner orders retrieved successfully", carService.getMyOrders(principal.getName())));
    }

    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<PartnerOrderResponseDto>>> getAllOrders() {
        return ResponseEntity.ok(ApiResponse.success("All orders retrieved successfully", carService.getAllOrders()));
    }
    ...
    public ResponseEntity<ApiResponse<PartnerOrderResponseDto>> updateOrder(@PathVariable Long id,
                                                                            ...
                                                                            Principal principal) {
        return ResponseEntity.ok(ApiResponse.success("Order updated successfully", carService.updateOrder(id, request, principal.getName())));
    }
    ...
    public ResponseEntity<ApiResponse<PartnerOrderResponseDto>> decideOrder(@PathVariable Long id,
        ...
        return ResponseEntity.ok(ApiResponse.success("Order decision saved successfully", carService.decideOrder(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('PARTNER', 'ADMIN')")
    public ResponseEntity<ApiResponse<Void>> deleteOrder(@PathVariable Long id, Principal principal) {
        carService.deleteOrder(id, principal.getName());
