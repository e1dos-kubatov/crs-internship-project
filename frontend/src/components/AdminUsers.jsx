...
const AdminUsers = () => {
  ...
  const deleteUser = async (user) => {
    if (!confirm(t('deleteUserConfirm', { email: user.email }))) return;
  ...
  const statusLabel = (user) => user.banned ? t('banned') : t('active');

  return (
    <div className="p-4 sm:p-8">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.25em] text-sky-700">{t('security')}</p>
          <h1 className="mt-2 text-3xl font-black text-slate-950">{t('backendUsers')}</h1>
        </div>
      ...
      <div className="mb-6 rounded-[2rem] border border-white/70 bg-white/85 p-5 shadow-xl backdrop-blur">
        <div className="grid gap-4 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
          ...
          <select value={providerFilter} onChange={(e) => setProviderFilter(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 font-bold capitalize outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100">
            {providers.map((provider) => <option key={provider} value={provider}>{provider === 'all' ? t('allProviders') : provider}</option>)}
          </select>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/85 shadow-xl backdrop-blur">
        {loading ? (
          <p className="p-8 text-slate-500">{t('loadingUsers')}</p>
        ) : filteredUsers.length === 0 ? (
          <p className="p-8 text-slate-500">{t('noUsersMatch')}</p>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('name')}</th>
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('email')}</th>
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('provider')}</th>
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('role')}</th>
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('status')}</th>
                  <th className="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{t('reason')}</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur">
          <div className="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-2xl">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.25em] text-amber-600">{t('banUser')}</p>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-bold text-slate-700">{t('bannedReason')}</span>
              <textarea
                value={banReason}
                onChange={(e) => setBanReason(e.target.value)}
                placeholder={t('writeBanReason')}
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button disabled={actionLoading} onClick={submitBan} className="rounded-2xl bg-amber-600 px-5 py-3 font-black text-white hover:bg-amber-700 disabled:opacity-60">
                {actionLoading ? t('banning') : t('banUser')}
              </button>
            </div>
          </div>
        </div>
