...
const homeBackgroundImage = 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&w=2400&q=85';
...
const Hero = () => {
  ...
  return (
    <>
      <section className="relative flex min-h-[600px] flex-col items-center px-4 pb-8 pt-12">
        <div className="absolute inset-0 z-0 overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-slate-950/70 via-slate-900/20 to-emerald-950/45"></div>
          <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-white via-white/50 to-transparent"></div>
        </div>

        <div className="relative z-10 mb-4 flex flex-wrap justify-center gap-3 text-xs font-black uppercase text-white">
          {t('heroChips').map((chip) => (
            <span key={chip} className="rounded-lg bg-white/15 px-4 py-2 backdrop-blur">{chip}</span>
          ))}
        </div>

        <h1 className="relative z-10 mb-4 max-w-4xl text-center text-3xl font-black leading-tight text-white md:text-5xl">
          {t('heroTitle')}
        </h1>

        <p className="relative z-10 mb-5 max-w-3xl text-center text-sm font-semibold leading-6 text-white/90 md:text-base">
          {t('heroLead')}
        </p>

        <div className="relative z-10 mb-5 rounded-lg border border-white/15 bg-white/15 px-5 py-3 text-sm font-bold text-white shadow-xl backdrop-blur">
          {t('cwdMeaning')}
        </div>

        <form onSubmit={handleSearch} className="relative z-10 w-full max-w-6xl rounded-lg border border-white/15 bg-slate-950/85 p-5 text-white shadow-2xl backdrop-blur-xl md:p-6">
          ...
          <div className="mt-5 flex flex-col items-center justify-end gap-4 border-t border-gray-700 pt-5 md:flex-row">
            <button type="submit" className="rounded-lg bg-gradient-to-r from-orange-500 to-emerald-600 px-10 py-3 text-sm font-black uppercase shadow-xl transition-all hover:-translate-y-0.5">
              {t('searchCars')}
            </button>
          </div>
        </form>
